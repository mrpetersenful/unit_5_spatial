...
# Now we are going to load in three right whale datasets. (This data is all publicly available, but it required me to combine multiple, messy, sources as well as data requests rom the North Atlantic Right Whale Consortium). We'll load:
...
USA_crit_hab_sf = USA_crit_hab_sf %>% dplyr::select(country, habitat, geometry) # drops all other variables from shapefile
...
# So that was a lot of hard work merging our USA and Canada datasets because their original formats were very different. Different file types (.csv vs. shapefile), different geometries (coordinates vs. polygons), different attributes and different CRSs. Now that we have combined these disparate datasets, we can use ggplot2 tools to turn this spatial vector data into maps!
...
bath_m_raw = marmap::getNOAA.bathy(lon1 = lon_bounds[1], lon2 = lon_bounds[2],
                                   lat1 = lat_bounds[1], lat2 = lat_bounds[2], resolution = 4)
